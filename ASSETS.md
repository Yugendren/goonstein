...
| assets/models/quaternius/elder_male.glb, smith_male.glb, child_female.glb, warden_male.glb | same four free "Standard" packs as above | CC0 (assets/models/quaternius/LICENSE.txt) | the vertical slice's cast, same merge: peasant or ranger outfit + base-body head + Universal Base Characters hairstyles (Hair_Long/Beard/Buzzed/Buns), repainted per character with merge.py --tint; warden_male.glb also carries the four KayKit clip names the enemy files ask for as merge.py --alias duplicates. 52-53 joints, see CATALOG.md |
