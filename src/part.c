// See part.h.
#include "part.h"
#include "level.h"   // level_tex_from_name / level_tex_name (level.h does not include part.h, so no cycle)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
bool part_load(PartDoc *d, const char *path) {
    ...
    while (*cur) {
        ...
        int nend = nt;   // the numbers run to the end of the line, or to the optional `tex` suffix
        for (int i = 2; i < nt; i++) if (strcmp(tok[i], "tex") == 0) { nend = i; break; }
        float f[12] = { 0, 0, 0, 1, 1, 1, 0, 0, 0, 1, 1, 1 }; int nf = 0;
        for (int i = 2; i < nend && nf < 12; i++) f[nf++] = (float)atof(tok[i]);
        p->pos = v3(f[0], f[1], f[2]); p->size = v3(f[3], f[4], f[5]); p->yaw = f[6]; p->pitch = f[7]; p->roll = f[8]; p->tint = v4(f[9], f[10], f[11], 1);
        p->tex = -1; p->tex_tile = 1;   // memset zeroed them, and 0 would mean TEX_STONE at tile 0
        if (nend + 1 < nt) {
            int t = level_tex_from_name(tok[nend + 1]);
            if (t < 0) SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "%s:%d unknown texture '%s'", path, ln, tok[nend + 1]);
            else p->tex = t;
            if (nend + 2 < nt) { char *end = NULL; float v = (float)SDL_strtod(tok[nend + 2], &end); if (end != tok[nend + 2]) p->tex_tile = v; }
        }
        d->n++;
    }
    SDL_free(text);
...
bool part_save(const PartDoc *d, const char *path) {
    ...
    fprintf(f, "# part: piece FILE  x y z  sx sy sz  yaw pitch roll  [r g b]  [tex NAME [TILE]]   (see src/part.h)\n");
    if (d->collide > 0) {
    ...
    for (int i = 0; i < d->n; i++) {
        ...
        if (p->tex >= 0) fprintf(f, "  tex %s %.3f", level_tex_name(p->tex), p->tex_tile > 0 ? p->tex_tile : 1.0f);
        fprintf(f, "\n");
    }
    fclose(f);
    return true;
