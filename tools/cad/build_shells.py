...
Writes, for each shell NAME:
    ...
    assets/models/own/shell/NAME_GROUP.obj   the game meshes, one per material group
    assets/models/own/NAME.part              the .part that places and skins them
...
fabric (an air conditioner, a washing line, a bench) goes in
    ...
    tools/cad/shells/extras/NAME.part

which is appended verbatim. Edit that, not the generated file: rebuilding overwrites the .part
exactly as it overwrites the OBJ.
"""
...
def write_part(name, groups, spec, size, bases):
    """The .part that places every group's OBJ at the building's own origin and skins it."""
    skins = spec["skins"]
    missing = [g for g in groups if g not in skins]
    if missing:
    ...
    extra = os.path.join(EXTRAS, name + ".part")
    if os.path.exists(extra):
        with open(extra) as f:
            body = f.read().rstrip("\n")
        lines += ["", "# ---- from tools/cad/shells/extras/%s.part ----" % name, body]
...
def write_facade(name, fits):
    """assets/models/own/arch/NAME_facade.part -- the kit frames that dress this shell's holes.

    Generated from the same `Walls` calls that cut the holes, so a window frame and the reveal it
    sits in can never drift apart again. Anything on the facade that is not tied to an opening --
    a cornice run, a lamp, a colonnade -- is hand-written in tools/cad/shells/extras/ and
    appended here verbatim.
    """
    lines = [
        "# %s_facade -- generated by tools/cad/build_shells.py. DO NOT EDIT." % name,
        "# The kit's frames, seated IN the reveals the shell cuts: each piece is pushed back into",
        "# the wall by its own architrave projection, so the moulding finishes flush with the",
        "# plaster instead of standing on it. Placed in island.txt at the building's own origin",
        "# and yaw, so these are the building's local coordinates.",
    ]
    for f in fits:
    ...
    extra = os.path.join(EXTRAS, name + "_facade.part")
    if os.path.exists(extra):
        with open(extra) as fh:
            lines += ["", "# ---- from tools/cad/shells/extras/%s_facade.part ----" % name,
                      fh.read().rstrip("\n")]

    path = os.path.join(ROOT, "assets", "models", "own", "arch", name + "_facade.part")
...
def build(name, spec, tmp):
    ...
    for g in sorted(groups, key=order_key):
        rgb = MATERIALS[skins[g].mat] if g in skins else (0.78, 0.78, 0.78)
    ...
    specs = []
    for g in sorted(groups, key=order_key):
        mat = skins[g].mat if g in skins else "paint"
    ...
    part = write_part(name, groups, spec, size, bases)
    facade = write_facade(name, fits) if fits else None
    return dict(name=name, per=per, tris=sum(per.values()), budget=spec["tris"],
                size=size, part=part, facade=facade, fits=len(fits))
